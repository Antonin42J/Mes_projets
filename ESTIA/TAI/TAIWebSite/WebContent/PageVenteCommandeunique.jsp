...
<html>
    ...
    <body>
        ...
        <div class="corps">
            ...
            <c:forEach items="${ListeCommandetitre}" var="commande">
                <h2 class="titrepage">Commande : <c:out value="${commande.nomclient}"/></h2>
            </c:forEach>
            ...
            <div class="retour">
                <a class="btnretour" onclick="rtn()"> <i class="fa-solid fa-circle-arrow-left iconeretour"></i> Retour</a>
            </div>

            <div class="listecommande">
                <c:forEach items="${ListeCommandeunique}" var="commande">
                    <div class="listec">
                        <p class="nomclientcommande"><c:out value="${commande.nomclient}"/> </p>
                        <p class="adresseclient">Adresse: <c:out value="${commande.adresseclient}"/> </p>
                        <p class="refco">Référance commande : <c:out value="${commande.refco}"/> </p>
                        <p class="articleco">Articles de la commande : </p>
                        <div class="listecommandearticle">
                            <c:forEach items="${ListeCommandeArticleunique}" var="article">
                                <div class="listecarticle"> 
                                    <p class="nomarticle"> <c:out value="${commande.nomarticle}"/></p>
                                    <div class="caracarticle">
                                        <p class="refarticle">Référence article: <c:out value="${commande.refarticle}"/> </p>
                                        <p class="prixarticle">Prix : <c:out value="${commande.prixarticle}"/> </p>
                                        <p class="quantitearticle">Quantité : <c:out value="${commande.quantitearticle}"/></p>
                                    </div>
                                    <p class="prixtotalarticle">Prix total article : </p>
                                </div>
                            </c:forEach>
                        </div>
                        <div class="fincommande">
                            <p class="prixco"> Prix total de la commande : <c:out value="${commande.prixco}"/> </p>
                        </div>
                    </div>
                </c:forEach>
            </div>
